...
def login():
    ...
    if user and user.check_password(password):
        return (
            jsonify({"message": "Login successful", "user": user.user_to_json()}),
            200,
        )
    else:
...
def add_movie(title_query):
    ...
    movie = All_movies(
        movie_name=title, thumbnail=poster, genre=genre, year=release_date
    )
...
@app.route("/all-movies/genres/<genres>")
...
def get_movie_by_name(movie_name):
    movies = All_movies.query.filter(
        All_movies.movie_name.ilike(f"%{movie_name}%")
    ).all()
    return jsonify([movie.to_json() for movie in movies])
